...
// MetaFish – The NFTs Fishing game P2E on Blockchain
...
contract MetaFishToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable marketingAddress =
        payable(0x9ef6D7B52492adFeA434904a5A273e9BE6215D64);
    address public rewardPool = 0xb377dF5B764ca550e82F4EF60c5569643aFB5571;
    ...
    uint256 public blacklistValidUntil;
    ...
    constructor() {
        ...
        blacklistValidUntil = block.timestamp + 3 days;
    ...
    function swapETHForTokens(uint256 amount) private {
        ...
        try
            uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{
                value: amount
            }(0, path, rewardPool, block.timestamp)
        {} catch {
            emit Log("external call failed");
        }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(maxTxPercent > 0, "maxTxPercent must be greater than 0");
    ...
    function setMarketingDivisor(uint256 divisor) external onlyOwner {
        ...
        emit SetMarketingDivisor(divisor);
    }
    ...
    function setMarketingAddress(address _marketingAddress) external onlyOwner {
        ...
        emit SetMarketingAddress(_marketingAddress);
    }
    ...
    function multiBlacklist(address[] calldata addresses) external onlyOwner {
        require(block.timestamp < blacklistValidUntil, "blacklist is not availalbe anymore");
        for (uint256 i=0; i < addresses.length; i++) {
